...
var metaverse = require('./metaverse')
...
Transaction.fromBuffer = function (buffer, network = networks.bitcoin, __noStrict) {
  ...
  for (i = 0; i < voutLen; ++i) {
    ...
    if (coins.isETPNetwork(network)) {
        let attachment = metaverse.readAttachmentBuffer(buffer, offset);
        outRecord.attachment = attachment.attachment
        outRecord.attachmentBytes = buffer.slice(offset, attachment.offset)
...
Transaction.prototype.addOutput = function (scriptPubKey, value, attachment = false) {
  ...
  let attachmentBytes = metaverse.getAttachmentBuffer(attachment);
